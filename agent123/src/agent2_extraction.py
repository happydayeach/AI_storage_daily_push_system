...
def extract_event(article: RawArticle, llm_client: LLMClient, theme_config: dict) -> Optional[ExtractedEvent]:
    ...
    industry_metadata = config_loader.get_industries(theme_config)
    industries = "; ".join(
        f"{key}={metadata['label']}（{metadata['desc']}）"
        for key, metadata in industry_metadata.items()
    )
    general_industry_rule = (
        '存储相关但不属于上述三类的填 "general"（通用）；'
        if "general" in industry_metadata
        else ""
    )
    verticals = "; ".join(
        f"{key}={metadata['label']}（{metadata['desc']}）"
...
- "industry": 产业标签，从 [{", ".join(json.dumps(key, ensure_ascii=False) for key in industry_metadata)}] 选一个。flash/distributed/data-protection 分别对应闪存、分布式、数据保护三大产业；{general_industry_rule}只有与存储完全无关的才填 ""（但 relevant 已过滤无关文章，industry 应总有值）。判断标准：{industries}。
